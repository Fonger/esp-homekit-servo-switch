...
typedef enum { PERIOD_ON = 0, PERIOD_OFF = 1 } pwm_step_t;
...
void pwm_start() {
  ...
  if (!pwmInfo._onLoad) {
    debug(
      "Can't set timer with low duty and frequency settings, put duty at 0");
    pwmInfo.dutyCycle = 0;
  }
  ...
  if (pwmInfo.dutyCycle > 0 && pwmInfo.dutyCycle < UINT16_MAX) {
    ...
    for (uint8_t i = 0; i < pwmInfo.usedPins; ++i) {
      gpio_write(pwmInfo.pins[i].pin,
                 pwmInfo.reverse ? !pwmInfo.output : pwmInfo.output);
    }
  }
  debug("PWM started");
  pwmInfo.running = 1;
